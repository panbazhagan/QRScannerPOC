...
import com.example.demoverifyapp.R
import com.example.demoverifyapp.analyzer.AnalyzerSetting
import com.example.demoverifyapp.analyzer.QrCodeAnalyzer
        ...
        if (AnalyzerSetting.useMlKit) {
            ...
            analysisUseCase?.setAnalyzer(
                ...
                QrCodeAnalyzer { qrResult ->
                    previewView!!.post {
                        val intent = Intent()
                        intent.putExtra("TOTP", qrResult.text)
    ...
    private fun processImageProxy(
        ...
        barcodeScanner.process(inputImage)
            .addOnSuccessListener { barcodes ->
                barcodes.forEach {
                    val intent = Intent()
                    intent.putExtra("TOTP", it.rawValue)
                    setResult(RESULT_OK, intent)
                    Log.d("QRCodeAnalyzer", "Barcode scanned: ${it.rawValue}")
                    finish()
                }
            }
